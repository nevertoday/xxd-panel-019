---
name: xxd-panel-019
description: "Create XXD Panel 019 artwork from supplied photos in four selectable modes: photo above/illustration below, photo left/illustration right, transformed illustration alone, or a four-device wallpaper pack with either independent compositions or anchor-linked visual continuity. Supports exact 50/50 paired layouts, custom dimensions, and source-aware editorial copy. Use for the exact 019 style; never use it for collages or general free-form posters."
---
...
- Resolve one output mode before generation: `top-bottom` (photo above, illustration below), `left-right` (photo left, illustration right), `design-only` (only the transformed illustration is visible), or `wallpaper-pack` (phone, iPad, desktop, and watch wallpapers). If the user has not already specified a mode, ask the one concise selection question defined in the workflow and wait for the answer; never ask again when the mode is already clear.
...
10. Generate the transformed illustration alone at the exact planned design-frame size, using the full reference prompt plus the aesthetic motive lock, source observations, 3–5 colors, locked copy package, and the resolved mode block below. In `design-only`, this frame is the entire final canvas. For an `independent` wallpaper pack, generate all four device compositions separately from the original source. For a `linked` pack, generate the resolved anchor device first—iPad by default—from the original source, then open it and require it to pass the source-identity, aesthetic, copy, and safe-area gates before continuing. Generate each remaining device as a fresh target-size composition using both the original source and that same approved anchor.
...
scripts/compose_panel.py --source photo.png --design illustration.png \
    --out ~/Desktop/xxd-panel-019/IMG_4821-top-bottom/IMG_4821.png --layout top-bottom --canvas 3:4

# left-right: exact vertical seam, exact custom resolution
scripts/compose_panel.py --plan --layout left-right --size 2560x1440
scripts/compose_panel.py --source photo.png --design illustration.png \
    --out ~/Desktop/xxd-panel-019/IMG_4821-left-right/IMG_4821-lr.png --layout left-right --size 2560x1440

# design-only: no visible photo and no seam
scripts/compose_panel.py --design illustration.png \
    --out ~/Desktop/xxd-panel-019/IMG_4821-design-only/IMG_4821-design.png \
    --layout design-only --size 2048x2048

# wallpaper-pack: four separately generated assets, finalized separately
scripts/compose_panel.py --design phone.png --out ~/Desktop/xxd-panel-019/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-phone.png --layout design-only --size 1440x3200
scripts/compose_panel.py --design ipad.png --out ~/Desktop/xxd-panel-019/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-ipad.png --layout design-only --size 2048x2732
scripts/compose_panel.py --design desktop.png --out ~/Desktop/xxd-panel-019/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-desktop.png --layout design-only --size 3840x2160
scripts/compose_panel.py --design watch.png --out ~/Desktop/xxd-panel-019/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-watch.png --layout design-only --size 1024x1024
```
...
**Transformed illustration.** Generate it alone, filling the exact design frame reported by `--plan`, with no photograph and no unused placeholder space. Append this resolved mode block:
...
**Finalize.** Paired modes place the two assets into exact equal frames. `design-only` resizes/crops the transformed illustration to the final dimensions without adding a source panel. The script reports severe cover crops so the asset can be regenerated or extended instead.
...
Save every generated poster under `~/Desktop/xxd-panel-019/`. Create the root and task directory if they do not exist.
...
Do not relax the one-photo-per-output rule in ordinary modes, exact four-output rule in `wallpaper-pack`, exact equal split in paired modes, absence of the photo in source-hidden modes, faithful visible photography, or the photo-derived illustration unless the user explicitly asks to leave the 019 style.

## Provenance boundary

The packaging pattern was informed by [photo-abstract-editorial](https://github.com/ZzzLc0405/photo-abstract-editorial), but its prompts, example assets, adaptive proportions, abstract-memory aesthetic, and English-only title rules are not dependencies of this skill. The operative visual specification is only the local `xxd-panel-019` reference prompt derived from `019.md`.
