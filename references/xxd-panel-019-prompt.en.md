...
1. **TOP_BOTTOM:** source photo above, transformed illustration below; exact equal height. With no explicit size, each panel retains the complete source W×H and the final canvas is W×2H.
2. **LEFT_RIGHT:** source photo left, transformed illustration right; exact equal width. With no explicit size, each panel retains the complete source W×H and the final canvas is 2W×H.
3. **DESIGN_ONLY:** the source remains reference evidence but is not visible; the transformed illustration fills the canvas with no seam. With no explicit size, use the source W×H.
...
- Top-bottom: the photographic asset fits the upper frame and the illustration fits the lower frame.
- Left-right: the photographic asset fits the left frame and the illustration fits the right frame.
- Design-only: generate one illustration at the full final size. Do not enlarge a former lower-half asset afterward.
- Wallpaper-pack: generate four illustrations separately at their resolved sizes and aspect ratios. `INDEPENDENT` solves all four from the source photo; `LINKED` approves one anchor and lets the other three reference the original photo plus that same anchor. Both relationships share subject and 019 style DNA while solving subject position, geometric hierarchy, typography, and safe area for each device.

Sections 4 through 6 apply unchanged to the transformed illustration. Its 3–5 colors and locked copy still derive from the current photo. The illustration asset itself contains no extra photograph, seam, frame, or reserved area.
...
LAYOUT RULE: Fill DESIGN FRAME completely. Render no extra photograph, seam, frame, or reserved area inside the illustration.
